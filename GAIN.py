...
class GAIN_Discriminator(nn.Module):
    def __init__(self, input_dim):
        super(GAIN_Discriminator, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim * 2, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
...
def sample_hint(batch_mask, hint_rate=0.9):
    hint_mask = (torch.rand_like(batch_mask) < hint_rate).float()
    return batch_mask * hint_mask + 0.5 * (1.0 - hint_mask)


def evaluate_normal_reconstruction(netG, data_loader, scaler, device, mask_rate, data_dir, tag):
    ...
    with torch.no_grad():
        for batch_x, in data_loader:
            B, D = batch_x.shape
            mask = (torch.rand(B, D, device=device) > mask_rate).float()
            noise = torch.rand(B, D, device=device)
            x_corrupted = batch_x * mask + noise * (1.0 - mask)
            generated = netG(x_corrupted, mask)
            imputed = batch_x * mask + generated * (1.0 - mask)
...
def train_model(data_dir=TRAIN_NORMAL_DIR, tag=TRAIN_TAG):
    ...
    netD = GAIN_Discriminator(input_dim).to(device)
    optG = optim.Adam(netG.parameters(), lr=LR_G)
    optD = optim.Adam(netD.parameters(), lr=LR_D)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_x, in train_loader:
            ...
            generated = netG(x_corrupted, mask)
            imputed = batch_x * mask + generated * (1.0 - mask)
            d_prob = netD(imputed, hint)
            loss_g_adv = -torch.mean((1.0 - mask) * torch.log(d_prob + 1e-8))
            loss_g_mse = torch.mean(((1.0 - mask) * (generated - batch_x)) ** 2) / (torch.mean(1.0 - mask) + 1e-8)
            loss_g = loss_g_adv + ALPHA * loss_g_mse
            optG.zero_grad()
            loss_g.backward()
            optG.step()

            d_prob = netD(imputed.detach(), hint)
            loss_d = -torch.mean(mask * torch.log(d_prob + 1e-8) + (1.0 - mask) * torch.log(1.0 - d_prob + 1e-8))
            optD.zero_grad()
            loss_d.backward()
            optD.step()
        ...
        with torch.no_grad():
            for val_x, in val_loader:
                ...
                mask = (torch.rand(B, D, device=device) > MASK_RATE).float()
                noise = torch.rand(B, D, device=device)
                generated = netG(val_x * mask + noise * (1.0 - mask), mask)
                val_mse = torch.mean(((1.0 - mask) * (generated - val_x)) ** 2) / (torch.mean(1.0 - mask) + 1e-8)
                val_total += val_mse.item()
                val_batches += 1
        ...
        if avg_val < best_val:
            ...
            best_epoch = epoch + 1
            patience_counter = 0
            best_g_state = copy.deepcopy(netG.state_dict())
            best_d_state = copy.deepcopy(netD.state_dict())
        else:
            patience_counter += 1

        if (epoch + 1) % 10 == 0:
            print(f'Epoch {epoch + 1}/{EPOCHS} | D Loss: {loss_d_log / len(train_loader):.4f} | G Loss: {loss_g_log / len(train_loader):.4f} | Val MSE: {avg_val:.6f}')
        if patience_counter >= PATIENCE:
            print(f'\n🛑 早停触发！最优epoch: {best_epoch}, 最优验证MSE: {best_val:.6f}')
            break

    if best_g_state is not None:
        netG.load_state_dict(best_g_state)
        netD.load_state_dict(best_d_state)
    ...
    print(f'\n💾 模型已保存至: {model_path}')
    evaluate_normal_reconstruction(netG, test_loader, scaler, device, MASK_RATE, data_dir, train_tag)
    return model_path, netG, scaler


def load_model_for_test(model_path, device):
    checkpoint = load_checkpoint(model_path, device)
    netG = GAIN_Generator(checkpoint['input_dim']).to(device)
    netG.load_state_dict(checkpoint['netG_state_dict'])
    netG.eval()
    return netG, checkpoint['scaler']


def evaluate_attack_recovery(netG, scaler, target_data_dir=TEST_DIR, tag=TEST_TAG):
    ...
    with torch.no_grad():
        for i in range(0, x_tensor.shape[0], BATCH_SIZE):
            ...
            noise = torch.rand_like(batch_x)
            generated = netG(batch_x * batch_mask + noise * (1.0 - batch_mask), batch_mask)
            imputed = batch_x * batch_mask + generated * (1.0 - batch_mask)
...
def main():
    ...
    if RUN_MODE in ['all', 'train']:
        model_path, netG, scaler = train_model(TRAIN_NORMAL_DIR, TRAIN_TAG)
    else:
        model_path = MODEL_PATH if MODEL_PATH is not None else find_latest_model(MODEL_PREFIX)
        print(f'✅ 加载模型: {model_path}')
        netG, scaler = load_model_for_test(model_path, device)
    if RUN_MODE in ['all', 'test']:
        if RUN_MODE == 'all':
            netG, scaler = load_model_for_test(model_path, device)
        evaluate_attack_recovery(netG, scaler, TEST_DIR, TEST_TAG)
