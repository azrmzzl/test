...
SOURCE_TAG = '正常运行_1_2000时刻_正常数据'
TARGET_TAG = '母线8断路_1_2000时刻_正常数据'
ATTACK_TAG = '母线8断路_1_1000时刻_宽松'
...
def load_and_normalize_data(data_dir='dataset_split', tag=None, scaler=None, fit_scaler=True):
    """
...
def main(data_dir=SOURCE_NORMAL_DIR, tag=SOURCE_TAG):
    ...
    # 1. 准备数据
    data_all, scaler, source_tag = load_and_normalize_data(data_dir, tag=tag, scaler=None, fit_scaler=True)
...
def main_finetune(pretrained_model_path, target_data_dir=TARGET_NORMAL_DIR, tag=TARGET_TAG, scaler=None):
    ...
    data_all, scaler, target_tag = load_and_normalize_data(target_data_dir, tag=tag, scaler=scaler, fit_scaler=(scaler is None))
    input_dim = data_all.shape[1]
    X_tensor = torch.FloatTensor(data_all).to(device)
...
def visualize_gain_result_target(netG, test_loader, scaler, device, mask_rate, A_inc, num_buses, num_lines, max_abs_error_threshold=None):
    ...
    mask_flat = np.random.rand(*true_mat_filtered.shape) > mask_rate
    true_values = true_mat_filtered[mask_flat]
    recovered_values = pred_mat_filtered[mask_flat]
    ...
    plot_single_sample_comparison(pred_matrix=pred_mat_filtered, true_matrix=true_mat_filtered, sample_idx=0,
...
def visualize_gain_result(netG, test_loader, scaler, device, mask_rate=0.2, data_dir=SOURCE_NORMAL_DIR, tag=None, max_abs_error_threshold=None):
    ...
    plot_time_series_comparison(y_true=true_mat_filtered, y_pred=pred_mat_filtered, method_name='PG-WGAIN', feature_idx=0, plot_len=200)
    plot_single_sample_comparison(pred_matrix=pred_mat_filtered, true_matrix=true_mat_filtered, sample_idx=0, method_name='PG-WGAIN')
