...
def main_train():
    ...
    IS_SHUFFLE = False
    # IS_SHUFFLE = False if FIXED_MASK_MODE else True
    # ==========================================

    num_train = int(x_all.shape[0] * 1)
    train_loader = torch.utils.data.DataLoader(x_all[:num_train], batch_size=64, shuffle=IS_SHUFFLE)

    model = PI_GraphMAE(in_channels=4, hidden_channels=64, out_channels=4).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=50, verbose=True)

    print(f"\n>>> 开始训练 (FixedMask={FIXED_MASK_MODE}, Shuffle={IS_SHUFFLE})...")
    model.train()
    ...
    for epoch in range(1000):
        ...
        for batch_x in train_loader:
            ...
            # 1. MSE Loss (在归一化空间算，没问题)
            loss_mse = F.mse_loss(recon * mask, batch_x * mask)

            # 2. [核心修复] Physics Loss (在反归一化空间算)
            # 如果一开始 Physics Loss 很大导致不稳定，可以先乘一个小权重，比如 0.01
        ...
        if (epoch + 1) % 10 == 0:
            ...
            print(
                f"Epoch {epoch + 1:04d} | Avg Loss: {avg_loss:.6f} | MSE: {loss_mse.item():.6f} | Phy: {loss_phy.item():.6f} | LR: {lr:.6f}")

    # 保存
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    model_path = f'pi_graphmae_ieee57_fixed_{timestamp}.pth'
