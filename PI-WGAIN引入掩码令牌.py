...
# --- 核心控制参数 ---
NORMALIZE = True  # 【重要】是否进行归一化（True=归一化，False=直接使用原始数据）
...
# 3. 数据加载 (展平 + 归一化) - 保持不变
# ==========================================
def load_and_normalize_data(data_dir=None, samples_file=None, scaler=None, fit_scaler=True, normalize=True):
    """
    加载并归一化数据
    ...
    参数:
        ...
        scaler: 外部传入的Scaler对象，如果提供则复用
        fit_scaler: 是否重新拟合Scaler（True=拟合新数据，False=仅转换）
        normalize: 是否进行归一化（True=归一化，False=直接使用原始数据）
    
    返回:
        data_norm: 归一化后的数据（如果normalize=False则返回原始数据）
        scaler: MinMaxScaler对象（如果normalize=False则返回None）
    """
    if data_dir is None:
    ...
    # 【关键修复】根据 normalize 参数决定是否进行归一化
    if normalize:
        if scaler is None or fit_scaler:
            scaler = MinMaxScaler()
            data_norm = scaler.fit_transform(raw_data)
            print(f"    📊 Scaler 已拟合 (min shape: {scaler.min_.shape}, scale shape: {scaler.scale_.shape})")
        else:
            # 复用已有的 Scaler，只进行转换，不重新拟合
            data_norm = scaler.transform(raw_data)
            print(f"    📊 使用已有 Scaler 进行转换")
    else:
        # 不进行归一化，直接返回原始数据
        data_norm = raw_data.copy()
        scaler = None
        print(f"    ⚠️ 跳过归一化，直接使用原始数据")
...
def main():
    ...
    # 1. 准备数据 - 【关键修复】先划分再归一化，避免数据泄露
    raw_data_all, _ = load_and_normalize_data(
        data_dir=SOURCE_DATA_DIR, 
        samples_file=SOURCE_SAMPLES_FILE,
        scaler=None,
        fit_scaler=False,  # 这里只是加载原始数据，不进行归一化
        normalize=False  # 【重要】这里只加载原始数据，不归一化
    )
    
    # 【修复1】先划分数据集
    first_sample_raw = raw_data_all[0:1]
    remaining_data_raw = raw_data_all[1:]
    
    train_size = int(len(remaining_data_raw) * 0.9)
    
    train_raw = remaining_data_raw[:train_size]      # 训练集原始数据
    val_test_raw = remaining_data_raw[train_size:]    # 验证+测试集原始数据
    
    # 【修复2】只在训练集上拟合 Scaler（如果需要归一化）
    if NORMALIZE:
        scaler = MinMaxScaler()
        scaler.fit(train_raw)  # ✅ 只基于训练集拟合
        print(f"\n📊 Scaler 已在训练集上拟合完成")
        print(f"   训练集大小: {len(train_raw)} 样本")
        print(f"   验证+测试集大小: {len(val_test_raw)} 样本")
        
        # 【修复3】分别转换各数据集
        train_norm = scaler.transform(train_raw)
        val_test_norm = scaler.transform(val_test_raw)
        first_sample_norm = scaler.transform(first_sample_raw)
    else:
        scaler = None
        print(f"\n📊 数据集划分完成（未归一化）")
        print(f"   训练集大小: {len(train_raw)} 样本")
        ...
        val_test_norm = val_test_raw.copy()
        first_sample_norm = first_sample_raw.copy()
    
    # 转换为张量
    train_tensor = torch.FloatTensor(train_norm).to(device)
    val_test_tensor = torch.FloatTensor(val_test_norm).to(device)
    first_sample_tensor = torch.FloatTensor(first_sample_norm).to(device)
    ...
    # 从验证+测试集中再划分出独立的验证集和测试集
    val_size = int(len(val_test_tensor) * 0.5)  # 50% 验证，50% 测试
    val_tensor = val_test_tensor[:val_size]
    test_tensor = torch.cat([first_sample_tensor, val_test_tensor[val_size:]], dim=0)
    
    val_loader = DataLoader(TensorDataset(val_tensor), batch_size=BATCH_SIZE, shuffle=False)
    test_loader = DataLoader(TensorDataset(test_tensor), batch_size=BATCH_SIZE, shuffle=False)
    
    print(f"   最终划分: 训练集={len(train_tensor)}, 验证集={len(val_tensor)}, 测试集={len(test_tensor)}")
    ...
    if NORMALIZE and scaler is not None:
        ...
        # 如果不归一化，设置默认值使反归一化为恒等变换
        scaler_min = torch.zeros(1, input_dim, dtype=torch.float32, device=device)
        scaler_scale = torch.ones(1, input_dim, dtype=torch.float32, device=device)
    ...
    return model_path, scaler  # 【修复】返回模型路径和 Scaler 供微调函数调用


# ==========================================
# 4.5 [新增] 目标域变拓扑微调程序 (Frozen Fine-Tuning)
# ==========================================
def main_finetune(pretrained_model_path, source_scaler, target_data_dir=None, target_samples_file=None, target_topology_file=None):
    """
    目标域变拓扑场景冻结微调
    
    参数:
        pretrained_model_path: 预训练模型路径
        source_scaler: 源域的 Scaler 对象（必须传入，避免数据泄露）
    ...
    # 1. 加载目标域(新拓扑)的数据与新关联矩阵 - 【修复】复用源域 Scaler
    raw_target_data, _ = load_and_normalize_data(
        data_dir=target_data_dir, 
        samples_file=target_samples_file,
        scaler=source_scaler,  # ✅ 传入源域 Scaler
        fit_scaler=False,  # ✅ 不重新拟合，只转换
        normalize=NORMALIZE  # 【使用全局配置】根据配置决定是否归一化
    )
    
    # 使用源域的 Scaler 转换目标域数据（如果需要归一化）
    if NORMALIZE and source_scaler is not None:
        target_norm = source_scaler.transform(raw_target_data)
    else:
        target_norm = raw_target_data.copy()
    
    input_dim = target_norm.shape[1]
    X_tensor = torch.FloatTensor(target_norm).to(device)
    
    # 后续计算使用源域 Scaler 的参数
    scaler = source_scaler
    
    print(f"\n📊 目标域数据处理完成 ({'已归一化' if NORMALIZE else '未归一化'})")
    print(f"   目标域总样本数: {len(X_tensor)}")
...
def visualize_gain_result_target(netG, test_loader, scaler, device, mask_rate, A_inc, num_buses, num_lines):
    ...
    true_mat = np.concatenate(all_trues, axis=0)
    
    # 【修改】如果提供了 scaler 则进行反归一化，否则直接使用原始数据
    if scaler is not None:
        pred_mat = scaler.inverse_transform(pred_mat)
        true_mat = scaler.inverse_transform(true_mat)
        print(f"\n>>> 已对数据进行反归一化")
    else:
        print(f"\n>>> 数据未归一化，直接使用原始值")
...
if __name__ == "__main__":
    ...
    # 1. 基准场景预训练 (将返回最优模型的文件路径和 Scaler)
    best_model_path, source_scaler = main()
    ...
    if os.path.exists(TARGET_DATA_DIR):
        main_finetune(
            ...
            source_scaler=source_scaler,  # ✅ 传入源域 Scaler
            target_data_dir=TARGET_DATA_DIR,
            target_samples_file=TARGET_SAMPLES_FILE,
            target_topology_file=TARGET_TOPOLOGY_FILE
        )
