...
class PI_GraphMAE(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_heads=4):
        ...
        self.enc_mask_token = nn.Parameter(torch.zeros(1, 1, in_channels))  # 初始化为0更稳定
        ...
        # --- Decoder: GCN (降级为 GCN，更加鲁棒) ---
        # GCN 也是拓扑感知的，但比 GAT 更好训练
        self.decoder_gcn = GCNConv(hidden_channels, hidden_channels)
        self.decoder_head = nn.Linear(hidden_channels, out_channels)
    ...
    def forward(self, x, edge_index, mask=None):
        ...
        # 4) Decoding (GCN + Linear)
        # 使用 GCN 解码，利用邻居信息平滑恢复
        h_rec = self.decoder_gcn(h, edge_big)
        h_rec = F.elu(h_rec)
