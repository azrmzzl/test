...
def evaluate_normal_reconstruction(model, data_loader, scaler, device, mask_rate, data_dir, tag):
    ...
    with torch.no_grad():
        for batch_x, in data_loader:
            seq_x = batch_x.unsqueeze(1)
            mask = (torch.rand_like(seq_x) > mask_rate).float()
            outputs = model(seq_x * mask).squeeze(1)
            imputed = batch_x * mask.squeeze(1) + outputs * (1.0 - mask.squeeze(1))
...
def train_model(data_dir=TRAIN_NORMAL_DIR, tag=TRAIN_TAG):
    ...
    model = LSTMImputer(input_dim, HIDDEN_DIM).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_x, in train_loader:
            ...
            loss = torch.mean(((1.0 - mask_2d) * (outputs - batch_x)) ** 2) / (torch.mean(1.0 - mask_2d) + 1e-8)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for val_x, in val_loader:
                seq_x = val_x.unsqueeze(1)
                mask = (torch.rand_like(seq_x) > MASK_RATE).float()
                outputs = model(seq_x * mask).squeeze(1)
                mask_2d = mask.squeeze(1)
                val_loss = torch.mean(((1.0 - mask_2d) * (outputs - val_x)) ** 2) / (torch.mean(1.0 - mask_2d) + 1e-8)
...
def load_model_for_test(model_path, device):
    ...
    model = LSTMImputer(checkpoint['input_dim'], checkpoint.get('hidden_dim', 64)).to(device)
...
def evaluate_attack_recovery(model, scaler, target_data_dir=TEST_DIR, tag=TEST_TAG):
    ...
    with torch.no_grad():
        for i in range(0, x_tensor.shape[0], BATCH_SIZE):
            ...
            outputs = model((batch_x * batch_mask).unsqueeze(1)).squeeze(1)
