...
METHOD_NAME = 'GCN-AE'
MODEL_PREFIX = 'gcn_ae_baseline'

# ==========================================
# 2. 图结构、量测向量与图特征之间的转换
# ==========================================
DEFAULT_PMU_POSITION_IEEE57 = [4, 10, 15, 20, 24, 29, 32, 37, 41, 48, 54]  # 1-based


def load_pmu_positions(data_dir):
    pmu_file = os.path.join(data_dir, 'PMU_Position.mat')
    if os.path.exists(pmu_file):
        pmu = sio.loadmat(pmu_file)['pmu_position'].reshape(-1).astype(np.int64)
        return pmu - 1
    print('⚠️ 未找到 PMU_Position.mat，使用 IEEE-57 默认 PMU 配置。')
    return np.array(DEFAULT_PMU_POSITION_IEEE57, dtype=np.int64) - 1


class GraphMapper:
    def __init__(self, data_dir, tag, input_dim, num_buses=NUM_BUSES):
        self.data_dir = data_dir
        self.tag = tag
        self.input_dim = input_dim
        self.num_buses = num_buses
        self.pmu_pos = load_pmu_positions(data_dir)
        self.A_inc, _, self.num_lines = load_grid_topology(data_dir, tag=tag, num_buses=num_buses)
        topo_file = None
        tag_norm = _normalize_tag(tag)
        if tag_norm is not None and os.path.exists(os.path.join(data_dir, f'Topology_Branch_{tag_norm}.mat')):
            topo_file = os.path.join(data_dir, f'Topology_Branch_{tag_norm}.mat')
        else:
            topo_file = os.path.join(data_dir, 'Topology_Branch.mat')
        branch_data = sio.loadmat(topo_file)['branch_data']
        self.from_bus = branch_data[:, 0].astype(np.int64) - 1
        self.to_bus = branch_data[:, 1].astype(np.int64) - 1
        self.status = branch_data[:, 2] if branch_data.shape[1] > 2 else np.ones(self.num_lines)
        self.num_nodes = self.num_buses + self.num_lines
        self.in_channels = 6
        self._build_slices()
        self.adj_norm = self._build_adj_norm()

    def _build_slices(self):
        idx = 0
        self.idx_v = slice(idx, idx + self.num_buses); idx += self.num_buses
        self.idx_p = slice(idx, idx + self.num_buses); idx += self.num_buses
        self.idx_q = slice(idx, idx + self.num_buses); idx += self.num_buses
        self.idx_pf = slice(idx, idx + self.num_lines); idx += self.num_lines
        self.idx_qf = slice(idx, idx + self.num_lines); idx += self.num_lines
        n_pmu = len(self.pmu_pos)
        self.idx_theta = slice(idx, idx + n_pmu); idx += n_pmu
        self.idx_v_pmu = slice(idx, idx + n_pmu); idx += n_pmu
        self.i_from_mask = np.isin(self.from_bus, self.pmu_pos)
        self.i_to_mask = np.isin(self.to_bus, self.pmu_pos)
        self.branch_indices_from = np.where(self.i_from_mask)[0]
        self.branch_indices_to = np.where(self.i_to_mask)[0]
        n_if = len(self.branch_indices_from)
        n_it = len(self.branch_indices_to)
        self.idx_ire_from = slice(idx, idx + n_if); idx += n_if
        self.idx_iim_from = slice(idx, idx + n_if); idx += n_if
        self.idx_ire_to = slice(idx, idx + n_it); idx += n_it
        self.idx_iim_to = slice(idx, idx + n_it); idx += n_it
        self.expected_dim = idx
        if self.input_dim < self.expected_dim:
            raise ValueError(f'GCN-AE 图映射需要至少 {self.expected_dim} 维输入，当前为 {self.input_dim} 维。')

    def _build_adj_norm(self):
        adj = np.eye(self.num_nodes, dtype=np.float32)
        for l_idx in range(self.num_lines):
            if self.status[l_idx] == 1.0:
                b_from = int(self.from_bus[l_idx])
                b_to = int(self.to_bus[l_idx])
                l_node = self.num_buses + l_idx
                if 0 <= b_from < self.num_buses and 0 <= b_to < self.num_buses:
                    adj[b_from, l_node] = 1.0
                    adj[l_node, b_from] = 1.0
                    adj[b_to, l_node] = 1.0
                    adj[l_node, b_to] = 1.0
    ...
    def to_device_tensors(self, device):
        self.pmu_pos_t = torch.tensor(self.pmu_pos, dtype=torch.long, device=device)
        self.branch_from_t = torch.tensor(self.branch_indices_from, dtype=torch.long, device=device)
        self.branch_to_t = torch.tensor(self.branch_indices_to, dtype=torch.long, device=device)
        self.adj_norm_t = self.adj_norm.to(device)
        return self

    def vector_to_graph(self, flat):
        B = flat.shape[0]
        device = flat.device
        x = torch.zeros(B, self.num_nodes, self.in_channels, dtype=flat.dtype, device=device)
        x[:, :self.num_buses, 0] = flat[:, self.idx_v]
        x[:, :self.num_buses, 1] = flat[:, self.idx_p]
        x[:, :self.num_buses, 2] = flat[:, self.idx_q]
        x[:, self.pmu_pos_t, 3] = flat[:, self.idx_theta]
        x[:, self.pmu_pos_t, 4] = flat[:, self.idx_v_pmu]
        x[:, self.num_buses:, 0] = flat[:, self.idx_pf]
        x[:, self.num_buses:, 1] = flat[:, self.idx_qf]
        if len(self.branch_indices_from) > 0:
            x[:, self.num_buses + self.branch_from_t, 2] = flat[:, self.idx_ire_from]
            x[:, self.num_buses + self.branch_from_t, 3] = flat[:, self.idx_iim_from]
        if len(self.branch_indices_to) > 0:
            x[:, self.num_buses + self.branch_to_t, 4] = flat[:, self.idx_ire_to]
            x[:, self.num_buses + self.branch_to_t, 5] = flat[:, self.idx_iim_to]
        return x

    def graph_to_vector(self, x):
        B = x.shape[0]
        device = x.device
        flat = torch.zeros(B, self.input_dim, dtype=x.dtype, device=device)
        flat[:, self.idx_v] = x[:, :self.num_buses, 0]
        flat[:, self.idx_p] = x[:, :self.num_buses, 1]
        flat[:, self.idx_q] = x[:, :self.num_buses, 2]
        flat[:, self.idx_pf] = x[:, self.num_buses:, 0]
        flat[:, self.idx_qf] = x[:, self.num_buses:, 1]
        flat[:, self.idx_theta] = x[:, self.pmu_pos_t, 3]
        flat[:, self.idx_v_pmu] = x[:, self.pmu_pos_t, 4]
        if len(self.branch_indices_from) > 0:
            flat[:, self.idx_ire_from] = x[:, self.num_buses + self.branch_from_t, 2]
            flat[:, self.idx_iim_from] = x[:, self.num_buses + self.branch_from_t, 3]
        if len(self.branch_indices_to) > 0:
            flat[:, self.idx_ire_to] = x[:, self.num_buses + self.branch_to_t, 4]
            flat[:, self.idx_iim_to] = x[:, self.num_buses + self.branch_to_t, 5]
        return flat


# ==========================================
# 3. 不依赖 torch_geometric 的 GCN-AE 模型定义
# ==========================================
class DenseGCNLayer(nn.Module):
    ...
    def forward(self, x, adj_norm):
        h = torch.einsum('ij,bjf->bif', adj_norm, x)
        return self.linear(h)


class GCNAE(nn.Module):
    def __init__(self, in_channels=6, hidden_channels=128, latent_channels=64):
        super(GCNAE, self).__init__()
        self.enc1 = DenseGCNLayer(in_channels, hidden_channels)
        self.enc2 = DenseGCNLayer(hidden_channels, latent_channels)
        self.dec1 = DenseGCNLayer(latent_channels, hidden_channels)
        self.dec2 = DenseGCNLayer(hidden_channels, in_channels)
        self.act = nn.ReLU()

    def forward(self, x, adj_norm):
        h = self.act(self.enc1(x, adj_norm))
        h = self.act(self.enc2(h, adj_norm))
        h = self.act(self.dec1(h, adj_norm))
        return self.dec2(h, adj_norm)


def evaluate_normal_reconstruction(model, data_loader, scaler, device, mask_rate, mapper, data_dir, tag):
    ...
    with torch.no_grad():
        for batch_x, in data_loader:
            mask_flat = (torch.rand_like(batch_x) > mask_rate).float()
            recon_graph = model(mapper.vector_to_graph(batch_x * mask_flat), mapper.adj_norm_t)
            recon_flat = mapper.graph_to_vector(recon_graph)
            imputed = batch_x * mask_flat + recon_flat * (1.0 - mask_flat)
            all_preds.append(imputed.cpu().numpy())
...
def train_model(data_dir=TRAIN_NORMAL_DIR, tag=TRAIN_TAG):
    ...
    mapper = GraphMapper(data_dir, train_tag, input_dim).to_device_tensors(device)
    ...
    model = GCNAE(in_channels=mapper.in_channels).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_x, in train_loader:
            ...
            recon_graph = model(mapper.vector_to_graph(batch_x * mask_flat), mapper.adj_norm_t)
            recon_flat = mapper.graph_to_vector(recon_graph)
            loss = torch.mean(((1.0 - mask_flat) * (recon_flat - batch_x)) ** 2) / (torch.mean(1.0 - mask_flat) + 1e-8)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for val_x, in val_loader:
                mask_flat = (torch.rand_like(val_x) > MASK_RATE).float()
                recon_graph = model(mapper.vector_to_graph(val_x * mask_flat), mapper.adj_norm_t)
                recon_flat = mapper.graph_to_vector(recon_graph)
                val_loss = torch.mean(((1.0 - mask_flat) * (recon_flat - val_x)) ** 2) / (torch.mean(1.0 - mask_flat) + 1e-8)
    ...
    torch.save({
        ...
        'input_dim': input_dim,
        'in_channels': mapper.in_channels,
        'scaler': scaler,
        'train_tag': train_tag,
    ...
    print(f'\n💾 模型已保存至: {model_path}')
    evaluate_normal_reconstruction(model, test_loader, scaler, device, MASK_RATE, mapper, data_dir, train_tag)
    return model_path, model, scaler


def load_model_for_test(model_path, device):
    checkpoint = load_checkpoint(model_path, device)
    model = GCNAE(in_channels=checkpoint.get('in_channels', 6)).to(device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    return model, checkpoint['scaler'], checkpoint['input_dim']


def evaluate_attack_recovery(model, scaler, input_dim, target_data_dir=TEST_DIR, tag=TEST_TAG):
    ...
    mapper = GraphMapper(target_data_dir, tag, input_dim).to_device_tensors(device)
    ...
    with torch.no_grad():
        for i in range(0, x_tensor.shape[0], BATCH_SIZE):
            ...
            recon_graph = model(mapper.vector_to_graph(batch_x * batch_mask), mapper.adj_norm_t)
            recon_flat = mapper.graph_to_vector(recon_graph)
            imputed = batch_x * batch_mask + recon_flat * (1.0 - batch_mask)
...
def main():
    ...
    if RUN_MODE in ['all', 'train']:
        ...
        model, scaler, input_dim = load_model_for_test(model_path, device)
    if RUN_MODE in ['all', 'test']:
        if RUN_MODE == 'all':
            model, scaler, input_dim = load_model_for_test(model_path, device)
        evaluate_attack_recovery(model, scaler, input_dim, TEST_DIR, TEST_TAG)
