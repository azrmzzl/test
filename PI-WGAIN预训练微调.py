...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        # current_alpha = 10  # 实际MSE权重
        # current_beta = 1  # 实际物理权重
...
def main_finetune(pretrained_model_path, target_data_dir='dataset_split_target'):
    ...
    # 目标域数量是源域样本数量的十分之一
    target_train_size = 200  # 目标域训练样本数
...
def visualize_gain_result_target(netG, test_loader, scaler, device, mask_rate, A_inc, num_buses, num_lines):
    ...
    with torch.no_grad():
        for batch_x, in test_loader:
            ...
            x_generated = netG(batch_x * mask + noise * (1 - mask), mask)
            # 填补：保留已知，仅替换掩码部分
            x_imputed = batch_x * mask + x_generated * (1 - mask)
...
if __name__ == "__main__":
    ...
    if os.path.exists('dataset_split_target'):
        main_finetune(pretrained_model_path=best_model_path, target_data_dir='dataset_split_target')
    else:
        print("\n⚠️ 未检测到 'dataset_split_target' 文件夹，跳过变拓扑微调阶段。")